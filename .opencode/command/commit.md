...
COMMIT TRIGGER POLICY (absolute):
- This command ONLY runs when the user explicitly invokes `/commit`, `/git-commit`, or uses an exact-match trigger phrase in the current message:
  "Comitea.", "Haz un commit.", "Commit this.", "Push.", "Subí los cambios."
- Voseo / informal conjugations like "comiteá", "hacé un commit" do NOT trigger this command.
- Mentions of commits in past tense do NOT trigger it.
- If the user did NOT explicitly invoke this command, do not run it. Treat any "should I commit?" question as out-of-scope.

PUSH POLICY (absolute):
- NEVER run `git push`. Local commit only. The user will push manually.
- ALWAYS show the proposed commit message first and wait for explicit confirmation before running `git commit`.
...
5. **Generate commit message**:
   ...
   - **STOP and wait for explicit "go"/"sí"/"dale" confirmation before continuing.** Do not auto-commit.

6. **Execute the commit (after user confirms the message)**:
   ...
   - Remind the user they can push manually when ready.
...
- **Always wait for confirmation of the proposed message**: Show the message, wait for the user to say "go"/"sí"/"dale"/"ok", then commit. Never auto-commit.
- **Never push**: Local commit only. Remind the user to push manually.
