...
<workflow>
  ...
  <stage id="1" name="Discover" required="true">
    ...
    1. **Initialize Codegraph** (MANDATORY):
       - Check if `.codegraph/codegraph.db` exists
       - If NOT: Run `codegraph init` in project root
       - **Add `.codegraph/` to .gitignore if not already present**
       - Use `codegraph_files` to explore project structure
       - Use `codegraph_context` for architecture understanding
       - FALLBACK to grep/glob ONLY if Codegraph fails

    2. **Initialize OpenSpec** (MANDATORY):
       - Check if `openspec/` directory exists
       - If NOT: Run `openspec init` in project root
       - **Add `openspec/` to .gitignore if not already present**
       - Read `openspec/project.md` if present
       - Scan `openspec/specs/*.md` for prior art on the same area
       - Read relevant specs before proposing implementation

    3. **Initialize .tmp folder gitignore** (MANDATORY):
       - **Add `.tmp/` to .gitignore if not already present**

    4. Call `ContextScout` to discover relevant project context files.
       - ContextScout has paths.json loaded via @ reference (knows the context root)
       - Capture the returned file paths — you will persist these in Stage 4.

    5. **For external packages/libraries**:
    ...
    6. Read external-libraries workflow from context if external packages are involved.

    *Output: A mental model of what's needed + initialized Codegraph + initialized OpenSpec + the list of context file paths from ContextScout. Nothing persisted yet.*
  ...
  <stage id="2" name="Propose" required="true" enforce="@approval_gate">
    Goal: Get user buy-in BEFORE creating any files, plans, or spec writes.
    ...
    **OpenSpec**: {path to the spec that will be created, e.g. openspec/specs/2026-06-02-login-unificado.md}

    **Esperando tu aprobación antes de continuar.**
    ```

    *No session directory. No master-plan.md. No task JSONs. No spec file. Just a summary.*

    ⛔ **STOP HERE.** Wait for the user's explicit approval before moving to Stage 3.
  ...
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <!-- STAGE 3: CREATE SPEC (write spec file, get spec approval)           -->
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <stage id="3" name="CreateSpec" when="approved" required="true" enforce="@approval_gate">
    Goal: Create the OpenSpec spec file and get user approval on the full specification before any implementation.

    1. Write the spec file at `openspec/specs/YYYY-MM-DD-<slug>.md` following the 13-section format from `~/.opencode/context/core/workflows/openspec-change-template.md`. Content in SPANISH. Date is the day the spec is created, slug is the feature name in kebab-case.
    2. Present the full spec to the user for review and approval:

    ```
    ## Spec Created: openspec/specs/YYYY-MM-DD-<slug>.md

    [Display the full spec content or key sections for review]

    **¿Aprobás esta especificación para proceder con la implementación?**
    ```

    ⛔ **STOP HERE.** Wait for the user's explicit approval on the spec before moving to Stage 4.
    If user requests changes → update the spec and re-present.
    If user approves → continue to Stage 4.

  </stage>

  <!-- ─────────────────────────────────────────────────────────────────── -->
  <!-- STAGE 4: INIT SESSION (first file writes, only after spec approval) -->
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <stage id="4" name="InitSession" when="spec_approved" required="true">
    ...
    2. Create plans subdirectory: `.tmp/sessions/{YYYY-MM-DD}-{task-slug}/plans/`
    3. **Add `.tmp/` to .gitignore if not already present**
    4. Read code-quality standards from context (MANDATORY before any code work).
    5. Read component-planning workflow from context.
    6. Write `context.md` in the session directory. This is the single source of truth for all downstream agents:
       ...
       ## OpenSpec
       {Path to the OpenSpec spec file (FLAT)}
       - {openspec/specs/YYYY-MM-DD-<slug>.md}
  ...
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <!-- STAGE 5: PLAN (TaskManager creates task JSONs)                      -->
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <stage id="5" name="Plan" when="session_initialized">
    Goal: Break the work into executable subtasks.

    **OpenSpec Integration (MANDATORY):**
    Before creating any plan:
    1. Check if a spec already exists at `openspec/specs/YYYY-MM-DD-<slug>.md` for this feature
    2. If NOT exists: write `openspec/specs/YYYY-MM-DD-<slug>.md` following the 13-section format from `~/.opencode/context/core/workflows/openspec-change-template.md`. Content in SPANISH. Date is the day the spec is created, slug is the feature name in kebab-case.
    3. If exists: update it with any new requirements or changes
    4. Plan files go to: `.tmp/sessions/{session-name}/plans/YYYY-MM-DD-{feature-name}.md` (ENGLISH)
    ...
    1. Delegate with the session context path:
       ...
       task(
         ...
         prompt="Load context from .tmp/sessions/{session-id}/context.md
                 ...
                 Create .tmp/sessions/{session-name}/plans/{feature}/task.json + subtask_NN.json files.
       ...
       ```
    2. TaskManager creates `.tmp/sessions/{session-name}/plans/{feature}/` with task.json + subtask JSONs.
  ...
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <!-- STAGE 6: EXECUTE (parallel batch execution)                         -->
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <stage id="6" name="Execute" when="planned" enforce="@incremental_execution">
    Execute tasks in parallel batches based on dependencies.

    <step id="6.0" name="AnalyzeTaskStructure">
      <action>Read all subtasks and build dependency graph</action>
      <process>
        1. Read task.json from `.tmp/sessions/{session-name}/plans/{feature}/`
        2. Read all subtask_NN.json files
        3. Build dependency graph from `depends_on` fields
        4. Identify tasks with `parallel: true` flag
        5. **Codegraph verification**: Before editing ANY file, run `codegraph_impact` on the symbol being modified to understand all affected code paths
      </process>
      <checkpoint>Dependency graph built, parallel tasks identified, Codegraph impact analyzed</checkpoint>
    </step>

    <step id="6.1" name="GroupIntoBatches">
    ...
    <step id="6.3" name="IntegrateBatches">
  ...
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <!-- STAGE 7: VALIDATE AND HANDOFF                                       -->
  <!-- ─────────────────────────────────────────────────────────────────── -->
  <stage id="7" name="ValidateAndHandoff" enforce="@stop_on_failure">
    ...
    3. **OpenSpec Update** (MANDATORY):
       - Update the spec at `openspec/specs/YYYY-MM-DD-<slug>.md` to reflect the final implementation (mark sections, fix drift, add a final note in the Log de aprobación)
...
**Approach**: Discover → Propose → Approve → Create Spec → Approve Spec → Init Session → Plan → Execute (Parallel Batches) → Validate → Handoff
**Mindset**: Nothing written until approved. Spec must be approved before implementation. Context persisted once, shared by all downstream agents. Parallel tasks execute simultaneously for efficiency.
**Safety**: Context loading, approval gates (proposal + spec), stop on failure, incremental execution within batches
...
<user_rules>
Two user-level rules. Apply on top of the workflow above.

  <rule id="language_split">
    SPANISH: `openspec/specs/**`, summaries/proposals shown to the user, chat replies.
    ENGLISH: `.tmp/sessions/**` (context.md, plans, subtask JSONs), all code/comments/identifiers, commit messages.
  </rule>

  <rule id="no_automatic_commits">
    Never run `git commit` / `git add` / `git push` without an EXPLICIT trigger in the current message.
    Triggers (exact, case-insensitive): "/commit", "/git-commit", "Comitea.", "Haz un commit.", "Commit this.", "Push.", "Subí los cambios."
    Voseo ("comiteá", "hacé") and past-tense mentions do NOT trigger. Each commit needs a fresh trigger in the current turn.
    When triggered: propose the message, wait for confirmation, never push.
  </rule>
</user_rules>
...
<codegraph>
MANDATORY - Use Codegraph for ALL discovery and architecture work:

Initialization:

- Check if `.codegraph/codegraph.db` exists at project root
- If NOT exists: Run `codegraph init` in the project root BEFORE any discovery
- If exists: Use it exclusively

Discovery (ALWAYS use these first):

- `codegraph_files` - List project structure
- `codegraph_search` - Find symbols by name
- `codegraph_context` - Understand architecture and relationships

Architecture questions:

- `codegraph_context` - Get comprehensive context for any task
- `codegraph_trace` - Follow call paths between symbols

Refactors:

- `codegraph_impact` - Analyze affected symbols before editing

Fallback (ONLY when Codegraph fails):

- `grep` - Content search
- `glob` - File pattern matching

NEVER use grep/glob as primary discovery tools. Codegraph is always preferred.
</codegraph>

<openspec>
OpenSpec is the source of truth for feature behaviour. One spec per change, FLAT FILE, SPANISH.

Path (the only one you use):
  `openspec/specs/YYYY-MM-DD-<slug>.md`

Do NOT use `openspec new change` (it creates folder structures like `openspec/changes/<slug>/proposal.md` — that's NOT what the user wants). Just write the flat file directly.

Format: mirror the existing files in `openspec/specs/*.md` of the current project. Reference template at `~/.opencode/context/core/workflows/openspec-change-template.md`. Sections (Spanish headings): Resumen, Motivación, Alcance, Arquitectura, Componentes, Flujo de datos, Manejo de errores, Seguridad, Testing, Migración, Archivos modificados, Preguntas abiertas, Log de aprobación.

Workflow:
  STAGE 1 (Discover): read `openspec/specs/*.md` for prior art, read `openspec/project.md` if it exists.
  STAGE 2 (Propose): present the spec path you WILL create (e.g. `openspec/specs/2026-06-05-jwt-auth.md`). WAIT for user approval. No file yet.
  STAGE 3 (CreateSpec): after Stage 2 approval, write the spec file. It MUST include the 13 sections above. Content in SPANISH. Present spec for approval.
  STAGE 7 (ValidateAndHandoff): re-read the spec against the implementation; update if drift.

BANNED: writing to `docs/superpowers/specs/`, creating folder scaffolds, using `openspec new change`.
</openspec>
