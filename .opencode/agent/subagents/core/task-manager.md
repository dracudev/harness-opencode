...
<instructions>
  <workflow_execution>
    <stage id="0" name="ContextLoading">
      ...
      <process>
         1. **Initialize Codegraph** (MANDATORY):
            - Check if `.codegraph/codegraph.db` exists in project root
            - If NOT exists: Run `codegraph init` in project root
            - Use `codegraph_context` to understand project architecture
         ...
         2. **Initialize OpenSpec** (MANDATORY):
            - Check if `openspec/` directory exists in project root
            - If NOT exists: Create `openspec/config.yaml` and `openspec/changes/` directory
            - Read `openspec/project.md` if present
            - Check `openspec/changes/` for active work
            - Read relevant specs before planning

         3. Load task management context:
         ...
         4. Check current task state:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' .opencode/skills/task-management/scripts/task-cli.ts status
           ```

         5. If context bundle provided, load and extract:
            - Project coding standards
            - Architecture patterns
            - Technical constraints

         6. If context is insufficient, call ContextScout via task tool:
           ```javascript
           task(
    ...
    <stage id="1" name="Planning">
      ...
      <process>
        1. Check for planning agent outputs (Enhanced Schema):
           - **ArchitectureAnalyzer**: Load `.tmp/sessions/{session-name}/plans/{feature}/contexts.json` if exists
    ...
    <stage id="2" name="JSONCreation">
      ...
      <process>
         1. Create directory:
           `.tmp/sessions/{session-name}/plans/{feature-slug}/`
        ...
        5. Report creation:
           ...
           Location: .tmp/sessions/{session-name}/plans/{feature}/
    ...
    <stage id="4" name="Archiving">
      ...
      <process>
        ...
        2. If completed_count == subtask_count:
           ...
           - Move folder: `.tmp/sessions/{session-name}/plans/{feature}/` → `.tmp/sessions/{session-name}/plans/completed/{feature}/`
        ...
        3. Report:
           ...
           Feature: {feature}
           Completed: {timestamp}
           Location: .tmp/sessions/{session-name}/plans/completed/{feature}/
           ```
      </process>
...
<conventions>
  ...
  <structure>
    <directory>.tmp/sessions/{session-name}/plans/{feature}/</directory>
    <task_file>task.json</task_file>
    <subtask_files>subtask_01.json, subtask_02.json, ...</subtask_files>
    <archive>.tmp/sessions/{session-name}/plans/completed/{feature}/</archive>
  </structure>
...
<enhanced_schema_integration>
  ...
  <planning_agent_integration>
    <architecture_analyzer>
      <input_file>.tmp/sessions/{session-name}/plans/{feature}/contexts.json</input_file>
      <fields_extracted>
  ...
  <populating_enhanced_fields>
    <step_1>Check for planning agent outputs in .tmp/sessions/{session-name}/plans/, .tmp/planning/, .tmp/contracts/, docs/adr/</step_1>
