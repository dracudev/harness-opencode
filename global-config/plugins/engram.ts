...
const ENGRAM_BIN = process.env.ENGRAM_BIN ?? Bun.which("engram") ?? "/home/dracudev/.local/bin/engram"
...
export const Engram: Plugin = async (ctx) => {
  ...
  // Track whether memory instructions have already been injected into the
  // system prompt for the current session. Instructions are ~120 lines and
  // only need to be injected ONCE per session (system prompt persists across
  // turns). Reset on compaction so the new post-compaction agent receives them.
  let memoryInstructionsInjected = false
  ...
  return {
    ...
    "experimental.chat.system.transform": async (_input, output) => {
      // Only inject memory instructions ONCE per session.
      // The system prompt persists across turns within a session, so
      // re-injecting every message was wasteful (~120 lines each time).
      // The flag is reset on compaction so the new post-compaction agent
      // receives fresh instructions.
      if (memoryInstructionsInjected) return
      ...
      memoryInstructionsInjected = true
    },
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      // Reset the instruction-injection flag so the new post-compaction
      // agent receives fresh Engram memory instructions in its system prompt.
